...
class DatePickerFragment : AppCompatDialogFragment(), DatePickerDialog.OnDateSetListener {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val cal = GregorianCalendar()                   // use the current date initially
        ...
        if (arguments != null) {
            ...
            val givenDate = arguments.getSerializable(DATE_PICKER_DATE) as Date?    // if we were passed a date, use it, otherwise leave the cal set ot the current date
            if (givenDate != null) {
                cal.time = givenDate
    ...
    override fun onDateSet(view: DatePicker, year: Int, month: Int, dayOfMonth: Int) {
        ...
        view.tag = dialogId    // pass the id back in the tag, to save the called storing their own copy
