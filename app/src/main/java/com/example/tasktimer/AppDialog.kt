...
//        fun onDialogCancelled(dialog: Int)
    }
    ...
    override fun onAttach(context: Context) {
        ...
        dialogEvents = try {    // Is there a parent fragment? If so, that will be what we call back
            parentFragment as DialogEvents
        } catch(e: TypeCastException) {
            try {       // No parent fragment, so call back the Activity instead
                context as DialogEvents
            } catch(e: ClassCastException) {    // Activity doesn't implement the interface
                throw ClassCastException("Activity $context must implement AppDialog.DialogEvents interface")
            }
        } catch(e: ClassCastException) {    // Parent fragment doesn't implement the interface
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        // fix "smart cast to Bundle is impossible, ..."
        ...
        return builder.setMessage(messageString).setPositiveButton(positiveStringId) { dialogInterface, which ->
            ...
            dialogEvents?.onPositiveDialogResult(dialogId, arguments)
                ...
                //callback negative result function, if it were to be implemented.
